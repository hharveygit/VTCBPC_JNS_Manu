%% Plots inflated pial rendering and subject T1 MRI with subject BPC-labeled stim sites for sub1 to include in Figure 2-1 (no CAR)
%
% Copied from plotInflatedT1slicesSub1, but uses the SNR table from the no-CAR outputs
%
...
snrTable = readtableRmHyphens(fullfile('output', 'CARsensitivityTest', sprintf('sub-%s', sub), sprintf('sub-%s_ch-%s_BPCSNR.tsv', sub, ch)), 'electrical_stimulation_site', 1);

%% Get destrieux labels for stim sites

% requires vistasoft
FSDir = fullfile('data', 'derivatives', 'freesurfer');

stimPairs = split(snrTable.electrical_stimulation_site, '-', 2);
pairXyzs = ieeg_getPairXyzs(stimPairs, elecs); 
labelsDestrieux = ieeg_getLabelXyzDestrieux(pairXyzs, fullfile(FSDir, sprintf('sub-%s', sub)));

labelsDestrieux(strcmp(snrTable.electrical_stimulation_site, 'LA1-LA2')) = {'Left_Amygdala'}; % in line with rest of analysis, LA1-LA2 manually reassigned to amyg
...
pairXyzsInf = getXyzsInfSimple(pairXyzs, hemi, gii, giiInf, 6, labelsDestrieux); % does not plot white matter electrodes or electrodes in wrong hemisphere
...
for vv = 1:3
    ...
    mkdir(fullfile('output', 'CARsensitivityTest', sprintf('sub-%s', sub), 'inflated'));
    saveas(f, fullfile('output', 'CARsensitivityTest', sprintf('sub-%s', sub), 'inflated', sprintf('sub-%s_ch-%s_infl_view%d_subBpcColors.png', sub, ch, vv)));
...
% Save
mkdir(fullfile('output', 'CARsensitivityTest', sprintf('sub-%s', sub), 'hippAmygSlices'));
kjm_printfig(fullfile('output', 'CARsensitivityTest', sprintf('sub-%s', sub), 'hippAmygSlices', sprintf('sub-%s_ch-%s_slice_subBpcColors', sub, ch)), [30, 10]);
