...
%
% This "Manu" manuscript version of the function is adapted from mnl_ieegBasics/functions/ieeg_getXyzMni.m, for the VTCBPC manuscript, such that the 
%   deformation path input can be specified, separate from the T1 nifti input.
%
...
%       t1Path =    char. The path to the T1w MRI nifti
%       defPath =   char. The path to the MNI 152 forward deformation nifti file.
...
%   HH 2021
%
function xyzMni = ieeg_getXyzMniManu(xyz, t1Path, defPath, rootdir, minDist)
    
    if ~exist('spm_vol.m','file')
        error('make sure spm is in your path')
    else
        t1Image = spm_vol(t1Path);
    end   
    ...
    dataOut = t1Image; % use input image as template for electrode image outputs
    for ii = 1:length(xyzStruct)
        
        idx = xyzStruct(ii).idx; % electrode indices in current set
        vol = zeros(t1Image.dim); % volume to put electrodes in
        
        % transform xyz positions to (nearest) voxel indices
        xyz_ind = round([xyzStruct(ii).xyz ones(length(idx), 1)] * inv(t1Image.mat')); 
        xyz_ind(:, 4) = [];
        for jj = 1:length(idx)
    ...
    % SPM requires the forward deform file path to be given without the 'y_' prefix, so we take the convoluted steps of removing 'y_' from the file name
    d = dir(defPath);
    defPathEdit = fullfile(d.folder, d.name(3:end));
    ...
    for ii = 1:length(xyzStruct)
        ...
        job.subj.vol{1} = defPathEdit; % forward deformation = "y_warpPathEdited";
